...
A Windows application was built in C++17, using version 2.0.10 of the SDL library (\citeyear{SDL2}) for window creation and frame duration calculation. Version 0.9.9.6 of the GLM library (\citeyear{GLM}) was used to provide a 3D vector class and some functions for vector mathematics.
...
Notice the call to \verb|respawn()|. When particle reached a ground level \verb|1.0f|, this function would be called to reset the particle's position, randomise its brightness, and set its new velocity with a degree of randomness.

% Wordcount: include this
% \begin{minted}[fontsize=\footnotesize]{C++}
...
%   float magnitudeMultiplier = randf()*0.95f+0.05f;
%   velocityRandomness =
%     normalize(velocityRandomness)
%     * velocityRandomnessAmount
%     * magnitudeMultiplier;
...
A Vulkan 1.1 graphics system was created in graphics.cpp that consumes an array of \verb|Particle| structures containing 3D positions and scalar brightness values as shown:

This data 

\section{Optimisation \& Analysis}

\subsection{,multithread and rng}
