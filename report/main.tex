...
Improving the efficiency of \verb|updateOneParticle()| was overdue and the chosen approach to tackling this was to rewrite the update code in SIMD instructions. CPU instructions generally mutate a single data points per instruction, but SIMD instructions can mutate multiple data points per instruction. In this project, we perform these instructions through the use of intrinsics; functions that we can write in C++ that the compiler turns into specially optimised machine code, including SIMD instructions. SIMD and intrinsics are not available on all x86/64 processors. Intel's documentation on our Core i5 6400 processor 
