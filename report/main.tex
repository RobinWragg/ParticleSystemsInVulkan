...
The compiler, which runs code analysis and performs optimisations during the build, failed to notice that this parameter wasn't being mutated during the call to \verb|graphics::render()|, in which case it would have been automatically accessed by reference. The parameter's data was being copied into a new \verb|std::vector| at the top of this function, allocating and freeing over 1.5 MBs every time the function was called. The parameter was changed to a const reference,
...
Which prevented the parameter from being copied and allowed the program to run up to twice as fast as shown in figure \ref{fig:pass-by-value-reference}. The change gave a smaller speedup at lower particle counts due to the smaller size of the data that was being copied before solving this issue.
