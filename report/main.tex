...
The function that spawned the threads would wait for them to finish by calling their \verb|std::thread::join()| method before returning. After these changes a bug was discovered that caused particles to respawn with poor pseudo-random number generation (RNG) for velocities and brightnesses (Figure \ref{fig:randombug}).
...
To move on with development, \verb|rand()| and \verb|srand()| were replaced with \verb|mt19937| \citep{mt19937Docs}, an C++ standard library class that implements the Mersenne Twister RNG algorithm \citep{Matsumoto1998}awef.
