...
The function that spawned the threads would wait for them to finish by calling \verb|std::thread::join()| before returning to allow the frame to be rendered. A bug was discovered in the \verb|respawn()| code that caused poor randomisation of velocities (Figure \ref{randombug}).
