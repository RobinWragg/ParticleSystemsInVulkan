...
\caption{Amdahl's Law: Best-case execution time of a program that requires one second of CPU time, by CPU core count.}
\label{fig:cpu_cores}
\end{figure}
...
Where $speedup$ is the theoretical, overall speed increase of the entire task, $s$ is the speed increase of the part of the that is improved by better system resources (CPU core count in our case), and $p$ is the original proportion of execution time that the improved part of the task occupied. % TODO: Explain why, and explain and cite Amdahl's law

In the best case when $p = 1$,
