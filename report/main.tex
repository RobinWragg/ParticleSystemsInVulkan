...
An example of one of the intrinsics that are used in this project is \verb|_mm256_add_ps(__m256 a, __m256 b)| \citep{IntelIntrinsicsAVX}, which returns another \verb|__m256|. This is a datatype which contains eight contiguous 32-bit floats ($32\cdot8=256$), known as a \emph{vector}; literally an eight-dimensional vector of floats, hence why SIMD is sometimes referred to as \verb|vectorisation|. \verb|_mm256_add_ps(__m256 a, __m256 b)| takes a pair of these vectors, adds them together, and returns the result as another vector, enabling us to add eight pairs of floats together using only one instruction.

As stated, the structure of \verb|__m256| requires that eight floats that should have the same operation performed on them are packed contiguously in memory, which means we cannot use the normal array-of-\verb|vec3|s memory layout of $xyzxyzxyzxyz$ (known as \emph{Structure-of-Arrays} or SoA). This is because the project requires that the $x$, $y$, and $z$ components are computed differently, \emph{e.g.} gravity must only be applied to the $z$ component.

For eight-wide SIMD instructions, values must be packed in groups of eight such as $xxxxxxxxyyyyyyyyzzzzzzzz$
